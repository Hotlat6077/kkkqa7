...
from preprocess_data import raw, tintegral, cderiv, routliers, ndarray2list0, ndarray2list1
from scipy.signal import butter, filtfilt
from 去滑雪坡特征 import ski_slope
from time_colo import TimerDecorator
...
#def spectrum(data, fs, methods='raw'):
    ...
    #exec(f"blank_dict['out']=raw(signal,fs)")
    ...
    # 去滑雪坡特征 
    ski = ski_slope(Feaa,fs)
    # T = frequencyx(RawSignal=Feaa, SampleFraquency=fs)
    T = frequencyx(RawSignal=ski, SampleFraquency=fs)
    Fea1x, Fea1y = T.fftx()  # # 频率分辨率 频率幅值
    log_y = np.log(Fea1y + 0.0001)  # 频率幅值计算对数

    result = {}
    # 原信号数据
    #result['fea_y'] = Feaa*1000
    #fea_y = [round(num/10, 2) for num in feaa]
    #print("Feaa",Feaa)
    print("FeaaType",type(Feaa))
    if methods=='raw':

        result['fea_y'] = Feaa
    else:

        #result['fea_y'] = [float(x) * 1000.0 / 9.8 for x in Feaa]
        result['fea_y'] = [float(x) * 1000.0 for x in Feaa]
    #result['fea_y'] = fea_y
    length = len(Feaa)
    ...
    #result['fftyy'] = [float(x) * 1000.0 for x in Fea1y]
        result['fftyy'] = [float(x) for x in Fea1y]
    else:
        result['fftyy'] = [float(x) * 1000.0 for x in Fea1y]
