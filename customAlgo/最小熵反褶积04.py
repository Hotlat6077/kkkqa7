...
def update_mins_fj_signal_analysis_MED_cao_fix(data, fs, filetime='20251229180100', methods='raw'):
    ...
    fs = float(np.asarray(fs).flat[0]) if fs is not None else 0.0
    ...
    # medx 返回的 Feax 可能是 (n,1) 二维列向量，flatten 成 1D 以避免 fftx 中 map(float,y) 报错
    Feax = np.asarray(Feax).flatten()
    ...
    # med数据（Feax 已为 1D，用 flatten().tolist() 避免 ndarray2list1 对标量迭代报错）
    result['y2'] = np.asarray(Feax).flatten().tolist()
    length2 = len(result['y2'])
    f_x2 = (np.arange(length2) + 1).tolist()
    ...
    if len(largest_12) >= 1:
        ...
        indices, values = [], []
    # 根据索引获取对应频率
    positions = [Feax2[i] for i in indices]

    # 幅值和位置分别保留小数位数（先转为 Python 标量再 round，避免多维数组报错）
    formatted_values = [round(float(np.asarray(val).flat[0]), 6) for val in values]
    formatted_positions = [round(float(np.asarray(pos).flat[0]), 6) for pos in positions]
...
if __name__ == '__main__':
    ...
    query = {
        # 'sensorId':sesor_id,
        "reportTime": "20260109040100",
        "thingsModel": "wave_2_0_12",
        "serialNum": "1077691480608",
    }
    print("query",query)
