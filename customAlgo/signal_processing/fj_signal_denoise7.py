...
def ndarray2list1(data):
    ...
    for i in list0:
        for j in i:
            list1.append(j)
    return list1
def update_mins_fj_signal_denoise7(path,group,machine,component,sensor):
    db = get_db()
    ...
    result['yy1']=Feay.tolist()
    length=len(Feay)
    result['xx1']=ndarray2list0(np.arange(length)+1)
    # # 频谱数据
    Fea1x, Fea1y = frequencyx(RawSignal=Feay, SampleFraquency=fs).fftx()
