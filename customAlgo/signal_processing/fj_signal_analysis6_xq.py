...
work_path = os.getcwd()
local_data_path = work_path + r'/data/fj_local'
...
def update_mins_fj_signal_analysis6_cao_fix(group, machine, component, sensor, methods, file_name):
    ...
    group = int(group)  #
    machine = int(machine)  #
    component = int(component)  # data/fj_local/1/1/1/1/20251128140500.csv
    sensor = int(sensor)  #
    fs = 1000  #
    folder_path = 'F:/sanaIOT/data/fj_local' + f"/{group}/{machine}/{component}/{sensor}"
    signal = []
    ...
    # signal = env(signal, fs)
    blank_dict = {}
    exec(f"blank_dict['out']={methods}(signal,fs)")  # 函数名传参
    ...
    result['fea_y'] = Feaa
    length = len(Feaa)
    f_x1 = ndarray2list0(np.arange(length) + 1)  # 生成一个range数据 从0 - length +1

    f_x = [x / fs for x in f_x1]
    f_x = [round(num, 2) for num in f_x]
    result['fea_x'] = f_x  # 最后的x轴就是这个吧
    ...
    ##############显示报警结论
    # collectionf = 'db['']'
    # collectionfre = 'eval(f'')'
    # fault_txtn = ['list(collectionfre.find({}, {'': 1}))']
    ...
    # ########0422 speed = 0.
    # 这里就是页面展示的内容 数据
    #  轴承内圈': np.float64(0.0), '外圈': np.float64(0.0), '滚动体': np.float64(0.0), '保持架': np.float64(0.0)
...
if __name__ == '__main__':
    ...
    """
    update_mins_fj_signal_analysis6_cao_fix(1, 1, 1, 1, 'raw', '20251128140500.csv')
