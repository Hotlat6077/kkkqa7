...
from index_calculation import frequency_index_x
import config
...
def update_mins_fj_signal_analysis_14(path, group, machine, component, sensor, file, localfilename, methods):
    db = get_db()
    ...
    # cursor=list(collection_sensor.find({'sensorID':f'SE_{group}_{machine}_{component}_{sensor}','onlineflag':True},
    #                        {'samplingfrequency':1,'faultfre':1,'hardware':1}))[0]
    # fs,faultfre,hw=cursor.get('samplingfrequency'),cursor.get('faultfre'),cursor.get('hardware')
    #
    # fault_type = list(faultfre.keys())
    # fault_value = list(faultfre.values())
    #
    # if hw == 'B': ###计算轴承报警谐波
    #     faults = fault_type[:3]
    #     xiebo_value = [[value, value * 2] for value in fault_value]
    #     result['xiebo1'] = xiebo_value[0]
    #     result['xiebo2'] = xiebo_value[1]
    #     result['xiebo3'] = xiebo_value[2]
    #     result['xiebo4'] = xiebo_value[3]
    #
    # elif hw == 'G':###计算齿轮箱边频
    #     faults = fault_type[3:]
    #     f_s = fault_value[0]
    ...
    #     sub_f_r = abs(f_m - f_r)
    #
    #     side_value = [add_f_s, add_f_p, add_f_r, sub_f_s, sub_f_p, sub_f_r, f_m]
    #     result['side'] = side_value
...
# 曹学勇修补
def update_mins_fj_signal_analysis_14_cao_fix(group, machine, component, sensor, methods, file_name):
    db = get_db()
    group = int(group)
    machine = int(machine)
    component = int(component)
