...
def get_time_domain_features(data):
    ...
    for i in range(len_):
        x_rms += data[0, i] ** 2  # 7.均方根值
        absXbar += abs(data[0, i])  # 8.平均绝对值
        x_r += math.sqrt(abs(data[0, i]))  # 9.方根幅值
        S += (data[0, i] - mean_[0]) ** 3  # 10.偏度
    ...
    fea = [rms, peak, kur, pulse, var, ske] # 17. 1.均方根值 2.峰值指标 3.峭度 4.脉冲指标 5.方差 6.偏斜度
    return fea
