...
def update_mins_fj_signal_analysis_waveletd(path, group, machine, component, sensor, file, localfilename, methods):
    import numpy as np
...
# 曹学勇修补 type raw
def update_mins_fj_signal_analysis_waveletd_cao_fix(group, machine, component, sensor, methods, file_name):
    import numpy as np
    db = get_db()
    group = int(group)
    machine = int(machine)
