...
def update_mins_fj_signal_analysis2(path,group,machine,component,sensor, file, localfilename,methods):
    db = get_db()
...
#曹学勇修补
def update_mins_fj_signal_analysis2_cao_fix(group, machine, component, sensor, methods, file_name):
    db = get_db()
    group = int(group)
    machine = int(machine)
    component = int(component)
