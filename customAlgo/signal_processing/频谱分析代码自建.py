...
# # 故障诊断 2 频谱图
# def update_mins_fj_signal_analysis6(path, group, machine, component, sensor, file, localfilename, methods):
#     """
#     group_type=GR_1
#     &machine_type=MA_1_1
#     &component_type=CO_1_1_1
#     &sensor_type=SE_1_1_1_1
#     &localfilename=2025-12-22+14%3A05
#     &method_type=raw&real_data=
#     """
#     db = get_db()
#     collection = db['vibration_data']  # vibration 震动数据
#     group = int(group)
#     machine = int(machine)
...
#     file = int(file)
#     temp_subsignal = ''
#     if file == 2:  # 文件夹为2 是128k采样
#         temp_subsignal = '128K加速度波形'
#         fs = 2000 * 2.56
#         fend = 2000
#     elif file == 1:  # 文件夹为1 是256K采样
#         temp_subsignal = '256K加速度波形'
#         fs = 10000 * 2.56
#         fend = 10000
#     else:
#         temp_subsignal = '16k速度波形'  # 文件夹为3 是16k采样
#         fs = 1000 * 2.56
#         fend = 1000
#     # temp_subsignal = '128K加速度波形'
...
#     signal = (signal - np.mean(signal)).tolist()  # 将成numpy数组转换列表
#     # #
#
#     blank_dict = {}
#     exec(f"blank_dict['out']={methods}(signal,fs)")
#     Feaa = blank_dict['out'].tolist()  # 将numpy数组转换成列表
#     #
...
# 曹学勇修补 频谱图 代码
def update_mins_fj_signal_analysis6_cao_fix(group, machine, component, sensor, methods, file_name):
    db = get_db()  # get db
    group = int(group)  #
    machine = int(machine)  #
    component = int(component)  #
    ...
    signal = np.array(signal)  # 1. 处理成 np.array
    ...
    blank_dict = {}
    exec(f"blank_dict['out']={methods}(signal,fs)")  # 这是是要运算的传入raw方法的 函数
    Feaa = blank_dict['out'].tolist()  # 获取Feaa
    ...
    result['fea_y'] = Feaa
    length = len(Feaa)
    f_x1 = ndarray2list0(np.arange(length) + 1)  # 这里的长度是Feaa的数据长度
    f_x = [x / fs for x in f_x1]  #
    f_x = [round(num, 2) for num in f_x]  #
    result['fea_x'] = f_x
...
if __name__ == '__main__':
    ...
    # 自修改代码
    # 文档数据目录地址
    folder_path = r'F:\sanaIOT\data\fj_local\1\1\1\1\20251128140500.csv'
    # 保存信号的空列表 用来保存数据
    signal = []
    # 读取数据
    # with open(f"{folder_path}/{file_name}", 'r+') as f:
    with open(f"{folder_path}", 'r+') as f:
        data = f.readlines()[0]
        signal.extend(list(map(float, data.split(','))))  # 转成映射的浮点数
    # 对速度进行处理
    # print(signal)
    # 信号进行numpy数据处理 传进数据分析函数
    signal = np.array(signal)
    blank_dict = {}
    # fj_signal_analysis_HE_data()
