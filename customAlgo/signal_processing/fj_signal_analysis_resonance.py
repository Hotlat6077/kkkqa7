...
import os
from signal_processing.Signal6_denoise import denoisex
...
def update_mins_fj_signal_analysis_resonance(path, group, machine, component, sensor, file, localfilename, methods):
    db = get_db()
    ...
    speed=speed_array
    speed_list = speed_array.tolist()  # 最后一次性转为Python列表
    ###############
...
#曹学勇修补
def update_mins_fj_signal_analysis_resonance_cao_fix(group, machine, component, sensor, methods, file_name):
    db = get_db()
    group = int(group)
    machine = int(machine)
    component = int(component)
