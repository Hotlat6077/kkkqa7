...
## ----------------    0906 修改  引入参数多了一个localfilename
def update_mins_fj_signal_analysis3(path, group, machine, component, sensor, file, localfilename, methods):
    db = get_db()
    ...
    result['peak'] = ndarray2list1(Fea[:, 1])
    length = len(Fea[:, 0])
...
# 曹学勇修补
def update_mins_fj_signal_analysis3_cao_fix(group, machine, component, sensor, methods, file_name):
    db = get_db()
    collection = db['vibration_data']
    group = int(group)
    machine = int(machine)
