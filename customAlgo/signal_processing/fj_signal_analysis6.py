...
def update_mins_fj_signal_analysis6_cao_fix(group, machine, component, sensor, methods, file_name):
    ...
    # fs=fs['samplingfrequency']
    blank_dict = {}
    exec(f"blank_dict['out']={methods}(signal,fs)")  # 3. 这是是要运算的传入raw方法的 函数 只是把数据转成了数组
    Feaa = blank_dict['out'].tolist()  # 获取Feaa 4. 到list是什么意思

    T = frequencyx(RawSignal=Feaa, SampleFraquency=fs) # 创建一个算法实例
    Fea1x, Fea1y = T.fftx()  # 频谱分析
    log_y = np.log(Fea1y + 0.0001)  # 计算对数
