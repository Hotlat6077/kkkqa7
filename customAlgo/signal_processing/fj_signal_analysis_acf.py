...
def update_mins_fj_signal_analysis_acf(path,group,machine,component,sensor, file, localfilename,methods):
    db = get_db()
...
def update_mins_fj_signal_analysis_acf_cao_fix(group, machine, component, sensor, methods, file_name):
    db = get_db()
    group = int(group)
    machine = int(machine)
    component = int(component)
