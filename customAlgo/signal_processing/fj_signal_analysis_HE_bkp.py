...
def update_mins_fj_signal_analysis_HE_cao_fix(group, machine, component, sensor, methods, file_name):
    ...
    Feaa = blank_dict['out'].tolist()  # 2.

    T = frequencyx(RawSignal=Feaa, SampleFraquency=fs)  # 3.
    Feax, Feay = T.envelopex()  # 4.
    H = T.envelopexx()  # 5.
    result = {}
    # 原信号数据
    result['fea_y'] = H.tolist()  # 6.
    length = len(H)
    f_x1 = ndarray2list0(np.arange(length) + 1)  # 7.
    f_x = [x / fs for x in f_x1]  # 8.
    f_x = [round(num, 2) for num in f_x]  # 9.
    result['fea_x'] = f_x

    result['peak'] = Feay.tolist()  # 10.

    Feax = np.round(Feax, 3)  # 11.
    ...
    collectionf = db['sensor_data_2025']
    # #############显示报警结论
    collectionfre = eval(f'db.indicator_data_{group}_{machine}')  # 13.运行这个字串代码
    # 14.
    fault_txtn = list(collectionfre.find({'sensorID': f'SE_{group}_{machine}_{component}_{sensor}'}, {'faultstr': 1}))
    if len(fault_txtn) == 0:
        fault_txt = {}
    else:
        fault_txt = fault_txtn[-1]  # 取最后一个元素 #

    fault_txt = fault_txt.get('faultstr', "")  # 15. 取faultstr字段的值 如果没有，则返回空字符串
    result['fault_txt'] = fault_txt
    # #########0422
    frexx = str(theory_frequencies(group, machine, component, sensor, speed))  # 16. 这里的速度是取的假的么
    frexxx = frexx.replace("{", "").replace("}", "")
    frexxxx = frexxx.replace("'", " ")
    result['xxxx'] = frexxxx
    # ###########显示理论报警频率 这些是用在什么地方的？
    ...
    fault_fre = list(
        collectionf.find({'sensorID': f'SE_{group}_{machine}_{component}_{sensor}'}, {'faultfre': 1, 'hardware': 1}))
    if len(fault_fre) >= 1 and 'faultfre' in fault_fre[0]:
        fault_fre = fault_fre[0]['faultfre']
    else:
        fault_fre = {}
    # print('xxxx',fault_fre)
