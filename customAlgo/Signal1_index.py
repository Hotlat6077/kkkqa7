...
def get_time_domain_features(data):
    ...
    mean_ = data.mean(axis=1)[0]  # 1.均值
    var = data.var(axis=1)[0]  # 2.方差
    std_ = data.std(axis=1)[0]  # 3.标准差
    max_ = data.max(axis=1)[0]  # 4.最大值
    min_ = data.min(axis=1)[0]  # 5.最小值/
    ...
    peak = x_p / rms if rms != 0 else 0  # 11.峰值指标，避免除零
    pulse = x_p / abs(mean_) if mean_ != 0 else 0  # 12.脉冲指标，避免除零
    ske = S / ((len_ - 1) * std_ ** 3) if std_ != 0 else 0  # 14.偏斜度，避免除零
    kur = K / ((len_ - 1) * std_ ** 4) if std_ != 0 else 0  # 15.峭度，避免除零
