...
def _to_scalar(x):
    """将 numpy 数组或标量转为 Python 标量，避免 'only 0-dimensional arrays can be converted to Python scalars'"""
    a = np.asarray(x)
    if a.ndim == 0:
        return float(a)
...
# 曹学勇修补 对应 最小熵反褶积
def update_mins_fj_signal_analysis_MED_cao_fix(data, fs, filetime='20251229180100', methods='raw'):

    # 确保 fs 为 Python 标量，避免 "only 0-dimensional arrays can be converted to Python scalars"
    fs = _to_scalar(fs)
    ...
    permissions = {}
    filterSize = int(_to_scalar(permissions.get("filterSize", 5))) if permissions else 50
    termIter = int(_to_scalar(permissions.get("termIter", 1))) if permissions else 1
    termDelta = float(permissions.get("termDelta", 1)) if permissions else 1.0
    termDelta = _to_scalar(termDelta)
    T = gearboxx(RawSignal=Feaa, SampleFraquency=fs)
    Feax, Feay = T.medx(filterSize=filterSize, termIter=termIter, termDelta=termDelta)
    # Feax 可能是 (n,1) 二维或 list of arrays，展平为 1-D 再传给 frequencyx，避免 fftx 里 map(float,y) 报错
    Feax_flat = np.asarray(Feax).ravel()
    ...
    # med数据：Feax 可能是 list 或 (n,1) 数组，统一展平为 Python 标量列表
    result['y2'] = [_to_scalar(x) for x in np.asarray(Feax).ravel()]
    length2 = len(result['y2'])
    # result['fea_xaxis2'] = ndarray2list0(np.arange(length2) + 1)
    f_x2 = ndarray2list0(np.arange(length) + 1)
    f_x2 = [round(_to_scalar(x * 1000 / (fs / 4)), 2) for x in f_x2]
    result['fea_xaxis2'] = f_x2

    Z = frequencyx(RawSignal=Feax_flat, SampleFraquency=fs)
    Feax2, Feay2 = Z.fftx()
    # 频谱
    # Feax2=Feax2.astype(int)
    Feax2 = np.round(Feax2, 3)
    result['fea_xaxis3'] = [round(_to_scalar(v), 3) for v in Feax2]
    result['y3'] = [_to_scalar(v) for v in Feay2]
    ...
    if len(largest_12) >= 1:
        ...
        indices, values = [], []
    # 根据索引获取对应频率（转为 Python 标量，避免 numpy 标量导致的转换错误）
    positions = [_to_scalar(Feax2[i]) for i in indices]

    # 幅值和位置分别保留小数位数
    formatted_values = [round(_to_scalar(val), 4) for val in values]
    formatted_positions = [round(_to_scalar(pos), 2) for pos in positions]
    ...
    # speed 可能是 0 维/1 维数组，统一转为 Python 标量列表
    speed_arr = np.asarray(speed)
    result['speed'] = [float(speed_arr.ravel()[i]) for i in range(speed_arr.size)] if speed_arr.size else [0.0]
...
if __name__ == '__main__':
    ...
    update_mins_fj_signal_analysis_MED_cao_fix(data, fs, filetime='20251229180100', methods='raw')