...
# 曹学勇修补 介次分析
def update_mins_fj_signal_analysis_14_cao_fix(data, fs, methods = 'raw'):
    # db = get_db()
    # group = int(group)
    # machine = int(machine)
    ...
    f_x = [round(float(num), 6) for num in f_x]
    result['fea_x'] = f_x
    # 阶次分析

    # Feax=Feax.astype(int)
    Feax = np.round(Feax, 6)
    Feax = Feax.tolist()
    ...
    formatted_values = [round(float(num), 6) for num in largest_12]
    formatted_positions = [round(float(num), 6) for num in values_positions]
...
if __name__ == '__main__':
    ...
    res = update_mins_fj_signal_analysis_14_cao_fix(data, fs, methods='raw')
    # x轴为result['fea_xaxis'] 
    # y轴为result['fea_y']
