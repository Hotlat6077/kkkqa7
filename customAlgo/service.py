    ...
    try:
        ...
        if request.method == 'GET':
            ...
            if request.is_json:
                ...
                logger.info(f"请求参数 data:{request_data} time:{time_}")
...
if __name__ == '__main__':
    import os
    import logging
    # 抑制 Flask 开发服务器警告（无论如何都生效）
    logging.getLogger('werkzeug').setLevel(logging.ERROR)
    # 使用环境变量 USE_PRODUCTION_SERVER=1 时用 Waitress（无开发服务器提示，适合生产）
    # 需先安装: pip install waitress
    if os.environ.get('USE_PRODUCTION_SERVER', '').strip() == '1':
        try:
            import importlib.util
            _spec = importlib.util.find_spec("waitress")
            if _spec is not None and _spec.loader is not None:
                _waitress = importlib.util.module_from_spec(_spec)
                _spec.loader.exec_module(_waitress)
                logger.info("使用 Waitress 生产服务器 (0.0.0.0:6100)")
                _waitress.serve(app, host='0.0.0.0', port=6100, threads=6)
            else:
                raise ImportError("waitress not found")
        except ImportError:
            logger.warning("未安装 waitress，使用 Flask 开发服务器。生产环境请: pip install waitress")
