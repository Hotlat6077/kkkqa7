...
def plot_waveform(result, axs_x, axs_y, color='red', show_plot=True, label_name='peak'):
    """
    绘制波段图
    x轴为result['fea_xaxis'] 
    y轴为result['fea_y']
    ...
    try:
        ...
        print("y_data shape: ", len(x_data))
        # 绘制波形图
        # plt.plot(x_data, y_data, linewidth=1, color='blue', label='Waveform')
        plt.plot(x_data, y_data, linewidth=1, color=f'{color}', label=f'{label_name}')
