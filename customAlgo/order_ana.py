...
def order_analysis(vibration_signal, sampling_frequency, rpm):
    # 确保 rpm/sampling_frequency 为 Python 标量，避免 "only 0-dimensional arrays can be converted to Python scalars"
    rpm = float(np.asarray(rpm).ravel()[0])
    sampling_frequency = float(np.asarray(sampling_frequency).ravel()[0])
    rpm = [rpm] * 100
    rps = np.array(rpm)
