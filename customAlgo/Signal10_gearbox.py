...
class gearboxx():
    ...
    def medx(self,filterSize = None,termIter = None,termDelta = None):
        ...
        # 转为 Python 标量，避免 "only 0-dimensional arrays can be converted to Python scalars"
        L = int(np.asarray(filterSize).ravel()[0])
        termIter = int(np.asarray(termIter).ravel()[0])
        termDelta = float(np.asarray(termDelta).ravel()[0])
        ...
        kurtIter = []
        f[1] = 1
        # 循环调整最小熵滤波器系数（标量化比较，避免 "truth value of an array is ambiguous"）
        n = 0
        while n == 0 or (n < termIter and (float(np.asarray(kurt(y) - kurtIter[n - 1]).ravel()[0]) > termDelta)):
            # 输出信号
            y = filter_matlab(f,x)
            y = np.asarray(y)
...
def kurt(x):
    ...
    if x.size == 0:
        return 0.0
    # 统一转为 1D，避免 2D 导致 result 为多元素数组
    x = np.reshape(x, (-1,))
    n = len(x)
    if n <= 2:
        return 0.0
    numer = np.sum((x - np.mean(x)) ** 4)
    denom = np.std(x) ** 4
    if denom == 0 or not np.isfinite(denom):
        return 0.0
    result = (numer / (n - 2)) / denom
    # 确保返回 Python 标量，避免 "truth value of an array is ambiguous"
