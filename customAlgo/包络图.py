...
def update_mins_fj_signal_analysis_HE_cao_fix(data, fs, time='20251229180100', methods='raw'):
    ...
    result['fea_y'] = Feaa  # 6. H是T.envelopex()方法返回的
    ...
    f_x = [round(num, 5) for num in f_x]  # 9. f_x 是上面的f_x变成 2位小数 后装到列表里
    result['fea_x'] = f_x  # 10.fea_x可能是x轴
    # fea_x x轴坐标
    result['peak'] = Feay.tolist()  # 10.peak是峰值 表示包络的峰值么 bn i j im e 
    # 包络的频率坐标
    #Feax = np.round(Feax, 3)  # 11.这里是取3位小数
    Feax = np.round(Feax, 5)  # 11.这里是取3位小数
    ...
    # collectionf = db['sensor_data_2025']  # 数据库的表名 应该是 传感器数据表 sensor传感器
    # #############显示报警结论  这里是显示报警结论
    # collectionfre = eval(f'db.indicator_data_{group}_{machine}')  # eval是用字串的形式 运行这个代码 获取数据库
    # # 相刑兢 果然
    # # fault  故障 断层 缺点 错误 这是在数据库时查的这个 faultstr 
    # fault_txtn = list(
    ...
    # fault_fre = list(  # 获取数据库指定数据 工厂 设备 组件 传感器 
    ...
    # === 修改部分开始 ===  过滤掉的key是什么意思 o wftc i lk fcu knx i j ld o wq 
    ...
    #     # 添加其他所有键（除了已添加的'fm'）因为fm的key之前添加过了 rhf go wb i nr wq 
    #     ordered_keys.extend([k for k in filtered_keys if k != 'fm'])
    # else:
    #     ordered_keys = filtered_keys  # 没有就按原来的顺序添加 rhf ftjb i rhf wq r  w wf ep wq tc klj ih fn b yi p su kc tha b 
    ...
    # === 修改部分结束 === xkrn r ryc i tda wq r tf o wb i yt d hv jfd wb r tf o wq i bn klj yt d hv jfd 
    ...
    fault_frev = [round(num, 5) for num in selected_values]  # 获取fault_fre_values 2位小数点
    ...
    time_add = datetime.strptime(time.split('.')[0], '%Y%m%d%H%M%S').strftime('%Y-%m-%d %H:%M:%S')
    ...
    # formatted_values = [round(num, 4) for num in largecst_12]
    #formatted_positions = [round(num, 2) for num in values_positions]
    formatted_positions = [round(num, 5) for num in values_positions]
...
def plot_waveforms(x1, y1, x2, y2):
    """绘制波形图"""
    ...
    ax2.plot(x2, y2, linewidth=2, color='green', label='处理后数据')
...
if __name__ == "__main__":
    ...
    result = update_mins_fj_signal_analysis_HE_cao_fix(data, fs, time='20251229180100', methods='raw')
    print(result)
    fea_x = result['fea_x']
    fea_y = result['fea_y']
    x1 = result['fea_xaxis']
    y1 = result['peak']
    # plot_waveforms(x1,y1,x1,y1)
