    ...
    try:
        ...
        if request.method == 'GET':
            ...
            if request.is_json:
                ...
                # 新添加了signalType字段，用于区分加速度和速度
                signal_type = request_data.get('signalType')
                logger.info(f"请求参数 data:{request_data}")
                if signal_type is not None and signal_type == 'vel':
                    logger.success("速度波形")
                    data_process = 'tintegral'
                else:
                    logger.success("加速度波形")
                logger.info(f"数据预处理类型{data_process}")
                # ######################################################
                # 如果传入参数是速度
                # 则要展示加速度变换来的速度波形
                ...
                if analyse_method=='spectrum' and document is not None:
                    ...
                    logger.info(f"频谱图结果: {result['fea_y'][0:20]}")
