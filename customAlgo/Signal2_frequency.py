...
class frequencyx():
    ...
    def fftx(self):
        ...
        df = 1 / fs  # 采样间隔时间
        y = self.RawSignal[:]  # 相当于数据copy
        # print("y shape:",y.shape)
        print("y type:",type(y))
        # y = list(map(float, y))  # 取浮点数
        y = np.array(y)  # 变换成数组
        print("y shape:",y.shape)
        f_values = np.linspace(0.0, fs / 2.0, Sampling_points // 2)  # 等分数列
    ...
    def envelopex(self):
        ...
        fmax = 1000  # 选取频率范围：最大
        n = 1  # 选择分析列数
        m = len(self.RawSignal)  # 采样点数 未处理信号的长度 信号的点数
        print('m :',m)
        f = np.arange(0, m) * self.SampleFraquency / m  # 频域波形很坐标 ：频率
        f_half = f[0:int(np.round(m / 2))]  # 取一半
        fmin_number = int(np.round(fmin * m / self.SampleFraquency))  # 获取点数
        fmax_number = int(np.round(fmax * m / self.SampleFraquency))
